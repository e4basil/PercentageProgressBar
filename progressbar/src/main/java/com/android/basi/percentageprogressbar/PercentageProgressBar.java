...
public class PercentageProgressBar extends View {
  ...
  private void init(AttributeSet attrs, int defStyle) {
    ...
    if (attrs != null) {
      final TypedArray a =
          getContext()
              .obtainStyledAttributes(attrs, R.styleable.PercentageProgressBar, defStyle, 0);
      try {

        float tempSize =
            a.getDimensionPixelSize(
                R.styleable.PercentageProgressBar_ppb_text_size,
        ...
        mCirclePaintColor =
            a.getColor(R.styleable.PercentageProgressBar_ppb_progress_color, Color.BLACK);

        mSecondaryCirclePaintColor =
            a.getColor(
                R.styleable.PercentageProgressBar_ppb_secondary_progress_color, Color.LTGRAY);

        mTextPaintColor = a.getColor(R.styleable.PercentageProgressBar_ppb_text_color, Color.BLACK);
  ...
  /** Set the two paint object with supplied color for drawing. */
  ...
  protected void onDraw(Canvas canvas) {
    ...
    canvas.drawArc(mCircleRect, mStartAngle, 360, false, mSecondaryCirclePaint);

    canvas.drawArc(mCircleRect, mStartAngle, angle, false, mCirclePaint);
    ...
    if (!TextUtils.isEmpty(text)) {
      ...
      canvas.drawText(
          text,
          (getWidth() - mProgressPaint.measureText(text)) / 2.0f,
          (getWidth() - textHeight) / 2.0f,
          mProgressPaint);
    }

    //        canvas.drawCircle((mCircleRect.right - mCircleRect.left) / 2, (mCircleRect.bottom -
    // mCircleRect.top) / 2,
  ...
  private void updateRectAngleBounds() {
    ...
    mCircleRect.set(
        paddingLeft + mProgressBarThickness,
        paddingTop + mProgressBarThickness,
